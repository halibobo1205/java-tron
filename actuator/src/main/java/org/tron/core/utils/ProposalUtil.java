...
//      }
      ...
      case ALLOW_WITNESS_SORT_OPT: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_8_0)) {
          throw new ContractValidateException(
              "Bad chain parameter id [ALLOW_WITNESS_SORT_OPT]");
        }
        if (dynamicPropertiesStore.allowWitnessSortOpt()) {
          throw new ContractValidateException(
              "[ALLOW_WITNESS_SORT_OPT] has been valid, no need to propose again");
        }
        if (value != 1) {
          throw new ContractValidateException(
              "This value[ALLOW_WITNESS_SORT_OPT] is only allowed to be 1");
        }
        break;
      }
      default:
