...
public class ConfigLoader {
  ...
  // isolate=true: a constant call bound to a non-HEAD (solidity/PBFT/state-trie) repository
  // installs its snapshot into a thread-local view instead of the process-wide global, so it
  // cannot pollute the flags the block-processing path reads concurrently.
  public static void load(Repository repository, boolean isolate) {
    if (!disable) {
