...
final class GrpcNettyMaxConcurrentStreamsLimiter {
  ...
  private static final class EnforcingProtocolNegotiator
    ...
    public ChannelHandler newHandler(GrpcHttp2ConnectionHandler grpcHandler) {
      // grpc-java builds the connection directly, bypassing Netty's builder-side pre-ACK
      // enforcement, so the server advertises SETTINGS_MAX_CONCURRENT_STREAMS without
      // enforcing it locally before the client acknowledges SETTINGS
      // (CVE-2026-47244). Applying the limit here, before any client stream is accepted,
      // restores enforcement. Drop this class once grpc-java fixes it upstream:
      // https://github.com/grpc/grpc-java/issues/12930
