...
public abstract class Http2ClientStreamTransportState extends AbstractClientStream.TransportState {
  ...
  protected void transportHeadersReceived(Metadata headers) {
    logger.debug("transportHeadersReceived: {}", headers);
    ...
    if (transportError != null) {
      ...
      logger.debug("transportHeadersReceived with transportError: {}", transportError);
      return;
    }
    try {
      if (headersReceived) {
      ...
      if (httpStatus != null && httpStatus >= 100 && httpStatus < 200) {
        ...
        logger.debug("transportHeadersReceived received informational headers: {}", headers);
        return;
      }
      ...
      logger.debug("transportHeadersReceived completed successfully");
      inboundHeadersReceived(headers);
      logger.debug("inboundHeadersReceived completed successfully");
    } finally {
      if (transportError != null) {
  ...
  protected void transportDataReceived(ReadableBuffer frame, boolean endOfStream) {
    if (transportError != null) {
      ...
      logger.debug("transportDataReceived with transportError: {}", transportError);
    } else {
      if (!headersReceived) {
        frame.close();
        http2ProcessingFailed(
            ...
            false,
            new Metadata());
        logger.debug("transportDataReceived without headers received");
        return;
      }
      int frameSize = frame.readableBytes();
      inboundDataReceived(frame);
      logger.debug("transportDataReceived completed successfully");
      if (endOfStream) {
  ...
  protected void transportTrailersReceived(Metadata trailers) {
    logger.debug("transportTrailersReceived: {}", trailers);
    ...
    if (transportError != null) {
      ...
      http2ProcessingFailed(transportError, false, transportErrorMetadata);
      logger.debug("transportTrailersReceived with transportError: {}", transportError);
    } else {
      Status status = statusFromTrailers(trailers);
      stripTransportDetails(trailers);
      inboundTrailersReceived(trailers, status);
      logger.debug("transportTrailersReceived completed successfully");
    }
  }
  ...
  private static void stripTransportDetails(Metadata metadata) {
    logger.debug("Stripping transport details from metadata: {}", metadata);
    metadata.discardAll(HTTP2_STATUS);
    metadata.discardAll(InternalStatus.CODE_KEY);
    metadata.discardAll(InternalStatus.MESSAGE_KEY);
    logger.debug("Stripped transport details from metadata: {}", metadata);
  }
}
