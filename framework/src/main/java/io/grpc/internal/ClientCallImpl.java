...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
  ...
  ClientCallImpl(
    ...
    logger.debug("Using call executor: {}", this.callExecutor.getClass().getName());
    ...
    logger.debug("Created ClientCall for method {}", method.getFullMethodName());
  }
  ...
  static void prepareHeaders(
    ...
    logger.debug("Prepared headers: {}", headers);
  }

  @Override
  public void start(Listener<RespT> observer, Metadata headers) {
    try (TaskCloseable ignore = PerfMark.traceTask("ClientCall.start")) {
      logger.debug("Starting call for method {}", method.getFullMethodName());
      PerfMark.attachTag(tag);
      startInternal(observer, headers);
      logger.debug("Started call for method {}", method.getFullMethodName());
    }
  }
  ...
  private void startInternal(Listener<RespT> observer, Metadata headers) {
    ...
    if (context.isCancelled()) {
      ...
      logger.debug("Call cancelled by context before start");
      return;
    }
    applyMethodConfig();
    ...
    if (compressorName != null) {
      ...
      if (compressor == null) {
        ...
        logger.debug("Call failed to start due to unknown compressor: {}", compressorName);
        return;
      }
    } else {
    ...
    if (!deadlineExceeded) {
      ...
      logger.debug("Created new stream for method {}", method.getFullMethodName());
    } else {
      ...
      logger.debug("Not creating stream for method {} as deadline already exceeded",
  ...
  private void applyMethodConfig() {
    ...
    logger.debug("Applied method config: {}, updated CallOptions: {}", info, callOptions);
  }
  ...
  private final class CancellationHandler implements Runnable, CancellationListener {
    ...
    void setUp() {
      ...
      logger.debug("CancellationHandler set up");
      if (tearDownCalled) {
    ...
    void tearDown() {
      ...
      logger.debug("CancellationHandler tear down");
    }
    ...
    public void cancelled(Context context) {
      if (hasDeadline && contextIsDeadlineSource
        ...
        logger.debug("Call cancelled by context deadline");
        return;
      }
      stream.cancel(statusFromCancelled(context));
      logger.debug("Call cancelled by context");
    }

    @Override
    public void run() {
      stream.cancel(formatDeadlineExceededStatus());
      logger.debug("Call cancelled by call options deadline");
    }
  ...
  public void request(int numMessages) {
    try (TaskCloseable ignore = PerfMark.traceTask("ClientCall.request")) {
      ...
      logger.debug("Requested {} messages", numMessages);
    }
  }
  ...
  public void cancel(@Nullable String message, @Nullable Throwable cause) {
    try (TaskCloseable ignore = PerfMark.traceTask("ClientCall.cancel")) {
      ...
      logger.debug("Cancelled call with message: {}", message);
    }
  }
  ...
  public void halfClose() {
    try (TaskCloseable ignore = PerfMark.traceTask("ClientCall.halfClose")) {
      PerfMark.attachTag(tag);
      halfCloseInternal();
      logger.debug("Half-closed call for method {}", method.getFullMethodName());
    }
  }
  ...
  private void halfCloseInternal() {
    ...
    stream.halfClose();
    logger.debug("Half-closed the stream");
  }
  ...
  public void sendMessage(ReqT message) {
    try (TaskCloseable ignore = PerfMark.traceTask("ClientCall.sendMessage")) {
      ...
      logger.debug("Sent message for method {}", method.getFullMethodName());
    }
  }
  ...
  private void sendMessageInternal(ReqT message) {
    ...
    if (!unaryRequest) {
      stream.flush();
      logger.debug("Flushed the stream after sending message");
    }
  }
  ...
  public void setMessageCompression(boolean enabled) {
    ...
    logger.debug("Set message compression to {}", enabled);
  }
  ...
  public boolean isReady() {
    ...
    logger.debug("Checked isReady: {}", stream.isReady());
  ...
  private void closeObserver(Listener<RespT> observer, Status status, Metadata trailers) {
    try {
      ...
      logger.debug("Closed observer with status: {}", status);
  ...
  private class ClientStreamListenerImpl implements ClientStreamListener {
    ...
    private void exceptionThrown(Status status) {
      ...
      stream.cancel(status);
      logger.debug("Exception thrown in call: {}", status);
    }

    @Override
    public void headersRead(final Metadata headers) {
      try (TaskCloseable ignore = PerfMark.traceTask("ClientStreamListener.headersRead")) {
        logger.debug("Headers read for call start {}", method.getFullMethodName());
        PerfMark.attachTag(tag);
        ...
        final class HeadersRead extends ContextRunnable {
          ...
          public void runInContext() {
            try (TaskCloseable ignore = PerfMark.traceTask("ClientCall$Listener.headersRead")) {
              ...
              runInternal();
              logger.debug("headersRead executed: {}", method.getFullMethodName());
            }
          }
        ...
        logger.debug("Read headers for call end {}", method.getFullMethodName());
      }
    }
    ...
    public void messagesAvailable(final MessageProducer producer) {
      try (TaskCloseable ignore = PerfMark.traceTask("ClientStreamListener.messagesAvailable")) {
        logger.debug("Messages available for call start {}", method.getFullMethodName());
        PerfMark.attachTag(tag);
        final Link link = PerfMark.linkOut();
        final class MessagesAvailable extends ContextRunnable {
          ...
          public void runInContext() {
            try (TaskCloseable ignore =
              ...
              PerfMark.linkIn(link);
              runInternal();
              logger.debug("MessagesAvailable executed {}", method.getFullMethodName());
            }
          }
        ...
        logger.debug("Messages available for call end {}", method.getFullMethodName());
      }
    }
    ...
    public void closed(Status status, RpcProgress rpcProgress, Metadata trailers) {
      try (TaskCloseable ignore = PerfMark.traceTask("ClientStreamListener.closed")) {
        logger.debug("Call closed start  {} with status {}", method.getFullMethodName(), status);
        PerfMark.attachTag(tag);
        closedInternal(status, rpcProgress, trailers);
        logger.debug("Call closed end {}", method.getFullMethodName());
      }
    }
      ...
      final class StreamClosed extends ContextRunnable {
        ...
        public void runInContext() {
          try (TaskCloseable ignore = PerfMark.traceTask("ClientCall$Listener.onClose")) {
            ...
            PerfMark.linkIn(link);
            runInternal();
            logger.debug("StreamClosed executed {}", method.getFullMethodName());
          }
        }
    ...
    public void onReady() {
      ...
      try (TaskCloseable ignore = PerfMark.traceTask("ClientStreamListener.onReady")) {
        logger.debug("onReady for call start {}", method.getFullMethodName());
        PerfMark.attachTag(tag);
        final Link link = PerfMark.linkOut();
        ...
        final class StreamOnReady extends ContextRunnable {
          ...
          public void runInContext() {
            try (TaskCloseable ignore = PerfMark.traceTask("ClientCall$Listener.onReady")) {
              ...
              PerfMark.linkIn(link);
              runInternal();
              logger.debug("onReady executed {}", method.getFullMethodName());
            }
          }
        ...
        logger.debug("onReady for call end {}", method.getFullMethodName());
      }
    }
  }
}
