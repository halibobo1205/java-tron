...
public final class ClientCalls {
  ...
  public static <ReqT, RespT> RespT blockingUnaryCall(
    ...
    logger.debug("blockingUnaryCall: {}", method.getFullMethodName());
  ...
  private static <ReqT, RespT> void startCall(
      ClientCall<ReqT, RespT> call,
      StartableListener<RespT> responseListener) {
    logger.debug("ClientCalls.startCall");
    call.start(responseListener, new Metadata());
    responseListener.onStart();
    logger.debug("ClientCalls.startCall: call started");
  }
  ...
  private static final class CallToStreamObserverAdapter<ReqT>
    ...
    public void onNext(ReqT value) {
      logger.debug("CallToStreamObserverAdapter.onNext");
    ...
    public void onError(Throwable t) {
      logger.debug("CallToStreamObserverAdapter.onError", t);
    ...
    public void onCompleted() {
      logger.debug("CallToStreamObserverAdapter.onCompleted");
      call.halfClose();
      completed = true;
    }
    ...
    public boolean isReady() {
      logger.debug("CallToStreamObserverAdapter.isReady");
      return call.isReady();
    }
    ...
    public void setOnReadyHandler(Runnable onReadyHandler) {
      ...
      logger.debug("CallToStreamObserverAdapter.setOnReadyHandler: handler set");
    }
    ...
    public void disableAutoRequestWithInitial(int request) {
      ...
      logger.debug("CallToStreamObserverAdapter.disableAutoRequestWithInitial: disabled with initial {}", request);
    }
    ...
    public void request(int count) {
      ...
      logger.debug("CallToStreamObserverAdapter.request: requested {}", count);
    }

    @Override
    public void setMessageCompression(boolean enable) {
      call.setMessageCompression(enable);
      logger.debug("CallToStreamObserverAdapter.setMessageCompression: {}", enable);
    }

    @Override
    public void cancel(@Nullable String message, @Nullable Throwable cause) {
      call.cancel(message, cause);
      logger.debug("CallToStreamObserverAdapter.cancel: call cancelled");
    }
  }
  ...
  private static final class StreamObserverToCallListenerAdapter<ReqT, RespT>
    ...
    public void onHeaders(Metadata headers) {
      logger.debug("onHeaders: headers = {}", headers);
    }
    ...
    public void onMessage(RespT message) {
      ...
      logger.debug("onMessage: observer notified");
    }

    @Override
    public void onClose(Status status, Metadata trailers) {
      logger.debug("onClose: status = {}", status);
      if (status.isOk()) {
        observer.onCompleted();
      } else {
        observer.onError(status.asRuntimeException(trailers));
      }
      logger.debug("onClose: observer notified");
    }

    @Override
    public void onReady() {
      logger.debug("onReady call");
      if (adapter.onReadyHandler != null) {
        adapter.onReadyHandler.run();
      }
      logger.debug("onReady: handler run");
    }

    @Override
    void onStart() {
      logger.debug("onStart call: requesting {}", adapter.initialRequest);
      if (adapter.initialRequest > 0) {
        adapter.request(adapter.initialRequest);
      }
      logger.debug("onStart: requested {}", adapter.initialRequest);
    }
  }
  ...
  private static final class UnaryStreamToFuture<RespT> extends StartableListener<RespT> {
    ...
    public void onMessage(RespT value) {
      logger.debug("onMessage: value = {}", value);
      ...
      logger.debug("onMessage: value stored");
    }
    ...
    public void onClose(Status status, Metadata trailers) {
      ...
      logger.debug("onClose: future set");
    }

    @Override
    void onStart() {
      logger.debug("onStart: requesting 2");
      responseFuture.call.request(2);
      logger.debug("onStart: requested 2");
    }
  }
  ...
  private static final class BlockingResponseStream<T> implements Iterator<T> {
    ...
    private final class QueuingListener extends StartableListener<T> {
      ...
      public void onMessage(T value) {
        ...
        buffer.add(value);
        logger.debug("onMessage: added to buffer");
      }
      ...
      public void onClose(Status status, Metadata trailers) {
        ...
        logger.debug("onClose: added to buffer");
      }

      @Override
      void onStart() {
        logger.debug("onStart: requesting 1");
        call.request(1);
        logger.debug("onStart: requested 1");
      }
    }
  }
  ...
  private static final class ThreadlessExecutor extends ConcurrentLinkedQueue<Runnable>
    ...
    public void waitAndDrain() throws InterruptedException {
      logger.debug("waitAndDrain: waiting to drain runnables");
      throwIfInterrupted();
      ...
      logger.debug("waitAndDrain: drained runnables");
    }
    ...
    public void shutdown() {
      logger.debug("shutdown: shutting down executor");
      waiter = SHUTDOWN;
      Runnable runnable;
      while ((runnable = poll()) != null) {
        runQuietly(runnable);
      }
      logger.debug("shutdown: executor shut down");
    }

    private static void runQuietly(Runnable runnable) {
      String name = runnable.getClass().getSimpleName();
      logger.debug("{} runQuietly: running runnable", name);
      try {
        runnable.run();
      } catch (Throwable t) {
        logger.warn("Runnable threw exception", t);
      } finally {
        logger.debug("{} runQuietly: finished running runnable", name);
      }
    }
    ...
    public void execute(Runnable runnable) {
      String name = runnable.getClass().getSimpleName();
      logger.debug("{} execute: adding runnable", name);
      add(runnable);
      ...
      logger.debug("{} execute: added runnable and signalled", name);
    }
  }
  ...
  static final class ThreadSafeThreadlessExecutor extends ConcurrentLinkedQueue<Runnable>
    ...
    public <T> void waitAndDrain(Predicate<T> predicate, T testTarget) throws InterruptedException {
      try {
        ...
        logger.debug("waitAndDrain: woke up and drained");
    ...
    public <T> void waitAndDrainWithTimeout(boolean waitForever, long end,
      ...
      logger.debug("waiting to drain runnables, long = {}", end);
      throwIfInterrupted();
      Runnable runnable;
      ...
      logger.debug("waitAndDrainWithTimeout: drained runnables");
    }
    ...
    public void drain() throws InterruptedException {
      logger.debug("drain: draining runnables");
      throwIfInterrupted();
      Runnable runnable;
      ...
      logger.debug("drain: drained runnables");
    }

    private void signallAll() {
      logger.debug("signallAll: signalling all waiters");
      waiterLock.lock();
      try {
        waiterCondition.signalAll();
      } finally {
        waiterLock.unlock();
        logger.debug("signallAll: signalled all waiters");
      }
    }
    ...
    private static void runQuietly(Runnable runnable) {
      ...
      try {
        ...
        logger.warn("{} Runnable threw exception", name, t);
      } finally {
        logger.debug("{} runQuietly: finished running runnable", name);
      }
    }
