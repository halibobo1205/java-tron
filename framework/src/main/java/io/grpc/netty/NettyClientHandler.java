...
class NettyClientHandler extends AbstractNettyHandler {
  ...
  private void onHeadersRead(int streamId, Http2Headers headers, boolean endStream) {
    logger.debug("Received HEADERS frame for stream {}: endStream={}", streamId, endStream);
    ...
    if (streamId != Http2CodecUtil.HTTP_UPGRADE_STREAM_ID) {
      ...
      logger.debug("Headers for stream {}: {}", streamId, headers);
      ...
      if (shouldFail && endStream) {
        ...
        logger.debug("Rejected headers for stream {} due to size exceeding soft limit", streamId);
        return;
      } else if (shouldFail) {
        stream.transportReportStatus(Status.RESOURCE_EXHAUSTED
            .withDescription(
    ...
    logger.debug("Completed processing of HEADERS frame for stream {}", streamId);
  }
  ...
  private void onDataRead(int streamId, ByteBuf data, int padding, boolean endOfStream) {
    logger.debug("Received DATA frame for stream {} of length {} and padding {}", streamId, data.readableBytes(), padding);
    ...
    logger.debug("Completed processing of DATA frame for stream {}", streamId);
  }
  ...
  private class FrameListener extends Http2FrameAdapter {
    ...
    public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding,
      ...
      logger.debug("onDataRead called for stream {} with length {} and padding {}",
          streamId, data.readableBytes(), padding);
      NettyClientHandler.this.onDataRead(streamId, data, padding, endOfStream);
      logger.debug("onDataRead completed for stream {}, padding {}", streamId, padding);
      return padding;
    }
    ...
    public void onHeadersRead(ChannelHandlerContext ctx,
      ...
      logger.debug("onHeadersRead completed for stream {}", streamId);
    }
    ...
    public void onPingAckRead(ChannelHandlerContext ctx, long ackPayload) throws Http2Exception {
      ...
      if (ackPayload == flowControlPing().payload()) {
        ...
        if (p.payload() == ackPayload) {
          ...
          logger.warn(
              "Received unexpected ping ack. Expecting {}, got {}", p.payload(), ackPayload);
        }
      } else {
