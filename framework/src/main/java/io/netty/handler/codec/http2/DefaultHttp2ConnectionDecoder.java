...
public class DefaultHttp2ConnectionDecoder implements Http2ConnectionDecoder {
  ...
  private final class FrameReadListener implements Http2FrameListener {
    ...
    public int onDataRead(final ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding,
      ...
      logger.debug("onDataRead streamId: {}, data readableBytes: {}, padding: {}, endOfStream: {}",
      ...
      try {
        ...
        logger.debug("onDataRead ignoring data frame due to exception: {}", e.getMessage());
        throw e;
      } catch (Throwable t) {
        logger.error("onDataRead unexpected error: ", t);
      ...
      if (shouldIgnore) {
        ...
        logger.debug("onDataRead ignoring data frame for stream id {}", streamId);
      ...
      try {
        ...
        if (error != null) {
          logger.debug("onDataRead throwing error for stream id {}: {}", streamId, error.getMessage());
          throw error;
        }
        ...
        if (endOfStream) {
          logger.debug("onDataRead endOfStream for stream id {}", streamId);
          lifecycleManager.closeStreamRemote(stream, ctx.newSucceededFuture());
        }
        logger.debug("onDataRead processed {} bytes for stream id {}", bytesToReturn, streamId);
        return bytesToReturn;
      } catch (Http2Exception e) {
        ...
        bytesToReturn -= delta;
        logger.debug("onDataRead throwing Http2Exception for stream id {}: {}", streamId, e.getMessage());
        throw e;
        ...
        int delta = unconsumedBytes - unconsumedBytes(stream);
        bytesToReturn -= delta;
        logger.debug("onDataRead throwing RuntimeException for stream id {}: {}", streamId, e.getMessage());
        throw e;
      } finally {
        // If appropriate, return the processed bytes to the flow controller.
        flowController.consumeBytes(stream, bytesToReturn);
        logger.debug("onDataRead consumed {} bytes for stream id {}", bytesToReturn, streamId);
      }
    }
    ...
    public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency,
      ...
      logger.debug("onHeadersRead streamId: {}, streamDependency: {}, weight: {}, exclusive: {}, "
            + "padding: {}, endOfStream: {}, headers: {}",
          streamId, streamDependency, weight, exclusive, padding, endOfStream, headers);
      Http2Stream stream = connection.stream(streamId);
      ...
      if (shouldIgnoreHeadersOrDataFrame(ctx, streamId, stream, endOfStream, "HEADERS")) {
        logger.debug("onHeadersRead ignoring headers frame for stream id {}", streamId);
        return;
      }
      ...
      logger.debug("onHeadersRead delivered headers for stream id {}", streamId);
      // If the headers completes this stream, close it.
      if (endOfStream) {
        logger.debug("onHeadersRead endOfStream for stream id {}", streamId);
        lifecycleManager.closeStreamRemote(stream, ctx.newSucceededFuture());
      }
      logger.debug("onHeadersRead completed for stream id {}", streamId);
    }
    ...
    private void applyLocalSettings(Http2Settings settings) throws Http2Exception {
      ...
      if (maxConcurrentStreams != null) {
        connection.remote().maxActiveStreams((int) Math.min(maxConcurrentStreams, MAX_VALUE));
      }
  ...
  private final class PrefaceFrameListener implements Http2FrameListener {
    ...
    public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding, boolean endOfStream)
      ...
      logger.debug("onDataRead streamId: {}, data readableBytes: {}, padding: {}, endOfStream: {}",
          streamId, data.readableBytes(), padding, endOfStream);
      logger.debug("onDataRead completed for stream id {}", streamId);
    ...
    public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding,
                              boolean endOfStream) throws Http2Exception {
      verifyPrefaceReceived();
      logger.debug("onHeadersRead (no priority) streamId: {}, padding: {}, endOfStream: {}, headers: {}",
          streamId, padding, endOfStream, headers);
      internalFrameListener.onHeadersRead(ctx, streamId, headers, padding, endOfStream);
      logger.debug("onHeadersRead (no priority) completed for stream id {}", streamId);
    }
    ...
    public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency,
                              short weight, boolean exclusive, int padding, boolean endOfStream) throws Http2Exception {
      verifyPrefaceReceived();
      logger.debug("onHeadersRead (with priority) streamId: {}, streamDependency: {}, weight: {}, "
            + "exclusive: {}, padding: {}, endOfStream: {}, headers: {}",
          streamId, streamDependency, weight, exclusive, padding, endOfStream, headers);
      internalFrameListener.onHeadersRead(ctx, streamId, headers, streamDependency, weight,
          exclusive, padding, endOfStream);
      logger.debug("onHeadersRead (with priority) completed for stream id {}", streamId);
    }
    ...
    public void onPingRead(ChannelHandlerContext ctx, long data) throws Http2Exception {
      verifyPrefaceReceived();
      logger.debug("onPingRead data: {}", data);
      internalFrameListener.onPingRead(ctx, data);
      logger.debug("onPingRead completed for data {}", data);
    }

    @Override
    public void onPingAckRead(ChannelHandlerContext ctx, long data) throws Http2Exception {
      verifyPrefaceReceived();
      logger.debug("onPingAckRead data: {}", data);
      internalFrameListener.onPingAckRead(ctx, data);
      logger.debug("onPingAckRead completed for data {}", data);
    }
    ...
    public void onPushPromiseRead(ChannelHandlerContext ctx, int streamId, int promisedStreamId,
                                  Http2Headers headers, int padding) throws Http2Exception {
      verifyPrefaceReceived();
      logger.debug("onPushPromiseRead streamId: {}, promisedStreamId: {}, padding: {}, headers: {}",
          streamId, promisedStreamId, padding, headers);
      internalFrameListener.onPushPromiseRead(ctx, streamId, promisedStreamId, headers, padding);
      logger.debug("onPushPromiseRead completed for promised stream id {}", promisedStreamId);
    }

    @Override
    public void onGoAwayRead(ChannelHandlerContext ctx, int lastStreamId, long errorCode, ByteBuf debugData)
        throws Http2Exception {
      logger.debug("onGoAwayRead lastStreamId: {}, errorCode: {}", lastStreamId, errorCode);
      onGoAwayRead0(ctx, lastStreamId, errorCode, debugData);
      logger.debug("onGoAwayRead completed");
    }
    ...
    public void onWindowUpdateRead(ChannelHandlerContext ctx, int streamId, int windowSizeIncrement)
        throws Http2Exception {
      verifyPrefaceReceived();
      logger.debug("onWindowUpdateRead streamId: {}, windowSizeIncrement: {}",
          streamId, windowSizeIncrement);
      internalFrameListener.onWindowUpdateRead(ctx, streamId, windowSizeIncrement);
      logger.debug("onWindowUpdateRead completed for stream id {}", streamId);
    }
