...
public class UpdateWitnessServletTest extends BaseHttpTest {

  private UpdateWitnessServlet servlet;
  private final String ownerAddr = ByteArray.toHexString(new ECKey().getAddress());

  @Override
  protected void setUpMocks() throws Exception {
    servlet = new UpdateWitnessServlet();
    injectWallet(servlet);
    when(wallet.createTransactionCapsule(
        any(WitnessContract.WitnessUpdateContract.class),
        eq(Protocol.Transaction.Contract.ContractType.WitnessUpdateContract)))
        .thenReturn(new TransactionCapsule(MINIMAL_TX));
  }

  @Test
  public void testUpdateWitness() throws Exception {
    String jsonParam = "{"
        + "\"owner_address\": \"" + ownerAddr + "\","
        + "\"update_url\": \"746573745f75726c\""
        + "}";
