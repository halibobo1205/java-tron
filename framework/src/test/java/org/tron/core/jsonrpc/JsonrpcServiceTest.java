...
public class JsonrpcServiceTest extends BaseTest {
  ...
  public void testGetStorageAt() {
    ...
    // hex block number: state queries are enabled (stateRoot.switch = true in
    // config-test.conf), so the numeric block is accepted and the empty address fails next
    Exception e5 = Assert.assertThrows(Exception.class,
        () -> tronJsonRpc.getStorageAt("", "", "0x1"));
    Assert.assertEquals("invalid address", e5.getMessage());
