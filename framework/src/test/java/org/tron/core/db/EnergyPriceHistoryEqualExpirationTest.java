 ...
 * matching live execution order. Pins the descending-id tie-break in
 * {@link org.tron.core.store.ProposalStore#getSpecifiedProposals}: it fails if the tie-break is
 * dropped (a stable expiration-only sort yields ascending id and rebuilds the wrong price), but
 * it cannot detect a revert to the pre-fix never-zero comparator, which produced this same order
 * for two equal-expiration proposals; that form is rejected at compile time by
 * ComparatorNeverReturnsZero.
 */
