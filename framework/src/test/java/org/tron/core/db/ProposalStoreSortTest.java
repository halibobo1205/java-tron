...
public class ProposalStoreSortTest extends BaseTest {
  ...
  private void put(long id, long createTime, long expirationTime, State state, int paramKey) {
    ...
    proposalStore.put(capsule.createDbKey(), capsule);
  }
  ...
  public void getSpecifiedProposals_equalExpiration_breaksTiesByIdDescending() {
    ...
    // from the tail -- so the lowest id must be last. This pins the explicit descending-id
    // tie-break: it fails on a rewrite that drops the tie-break (e.g. comparingLong(expiration)
    // alone, where TimSort stability yields ascending id). It does NOT fail on the pre-fix
    // never-zero comparator, which coincidentally produced this same order; that form is blocked
    // at compile time by ComparatorNeverReturnsZero.
