...
public class JsonCompatibilityFuzzTest {
  ...
  public void testGetObjectPojo() {
    // POJO deserialization via MAPPER.treeToValue
    JSONObject obj = JSON.parseObject(
        "{\"inner\":{\"name\":\"test\",\"nullField\":null}}");
    TestPojo pojo = obj.getObject("inner", TestPojo.class);
    assertNotNull(pojo);
    assertEquals("test", pojo.getName());
    assertNull(pojo.getNullField());
    // missing key returns null
    assertNull(obj.getObject("nonexistent", TestPojo.class));
  }
  ...
  // =========================================================================
  // 75. Trailing comma tolerance (Fastjson default, common in HTTP request bodies)
  // =========================================================================

  @Test
  public void testParseObjectWithTrailingComma() {
    // Fastjson silently accepts trailing commas; Jackson needs ALLOW_TRAILING_COMMA
    JSONObject obj = JSONObject.parseObject("{\"owner_address\":\"abc\",}");
    assertEquals("abc", obj.getString("owner_address"));
  }

  @Test
  public void testParseArrayWithTrailingComma() {
    JSONArray arr = JSONArray.parseArray("[1,2,3,]");
    assertEquals(3, arr.size());
  }

  @Test
  public void testParseNestedTrailingComma() {
    JSONObject obj = JSONObject.parseObject("{\"a\":{\"b\":1,},\"c\":[1,],}");
    assertEquals(1, obj.getJSONObject("a").size());
    assertEquals(1, obj.getJSONArray("c").size());
  }
  ...
  // ---------------------------------------------------------------------------
  // Section 76-78: Stringified JSON auto-parse (Fastjson compat)
  // ---------------------------------------------------------------------------

  @Test
  public void testGetJSONArrayFromStringifiedValue() {
    // Fastjson auto-parses string values containing JSON arrays
    JSONObject obj = JSON.parseObject("{\"arr\": \"[1,2,3]\"}");
    JSONArray arr = obj.getJSONArray("arr");
    assertNotNull(arr);
    assertEquals(3, arr.size());
  ...
  @Test
  public void testGetJSONObjectFromStringifiedValue() {
    // Fastjson auto-parses string values containing JSON objects
    JSONObject obj = JSON.parseObject("{\"nested\": \"{\\\"a\\\":1,\\\"b\\\":2}\"}");
    JSONObject nested = obj.getJSONObject("nested");
    assertNotNull(nested);
    assertEquals(Integer.valueOf(1), nested.getInteger("a"));
    assertEquals(Integer.valueOf(2), nested.getInteger("b"));
  }

  @Test
  public void testGetJSONArrayFromStringifiedComplexArray() {
    // Simulates form-url-encoded -> Util.getJsonString -> packTransaction path
    String contractJson = "[{\"type\":\"TransferContract\",\"parameter\":{\"value\":{}}}]";
    JSONObject obj = new JSONObject();
    obj.put("contract", contractJson);
    JSONArray arr = obj.getJSONArray("contract");
    assertNotNull(arr);
    assertEquals(1, arr.size());
    JSONObject contract = arr.getJSONObject(0);
    assertEquals("TransferContract", contract.getString("type"));
  }

  // ---------------------------------------------------------------------------
  // Section 79-80: JSONArray index-based stringified JSON auto-parse
  // ---------------------------------------------------------------------------

  @Test
  public void testArrayGetJSONObjectFromStringifiedElement() {
    // Fastjson auto-parses stringified JSON objects at array indices
    JSONArray arr = new JSONArray();
    arr.add("{\"key\":\"value\",\"num\":42}");
    JSONObject obj = arr.getJSONObject(0);
    assertNotNull(obj);
    assertEquals("value", obj.getString("key"));
    assertEquals(Integer.valueOf(42), obj.getInteger("num"));
  }

  @Test
  public void testArrayGetJSONArrayFromStringifiedElement() {
    // Fastjson auto-parses stringified JSON arrays at array indices
    JSONArray arr = new JSONArray();
    arr.add("[1,2,3]");
    JSONArray nested = arr.getJSONArray(0);
    assertNotNull(nested);
